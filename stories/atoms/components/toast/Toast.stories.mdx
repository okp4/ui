...
<Story
  name="Overview"
  argTypes={{ isOpened: { control: false }, jsxElement: { control: false } }}
  args={{
    title: 'Transaction',
...
The `Toast` component has a flexible content which notably allows the user to indicate a `JSX element`, a `title` and a `description`.  
These three elements are optional and are not inter-related.
    ...
    {args => {
      ...
      return (
        ...
        >
          ...
          <div>
            <Button label="JSX only" onClick={handleState('jsx', true)} backgroundColor="success" />
            <Toast
              isOpened={Object.values(state).includes('jsx') ? state['isOpened'] : false}
              onOpenChange={handleState('jsx', false)}
              jsxElement={<img src={BotAnikImage} width="100" />}
              severityLevel="success"
            />
          </div>
          <div>
            <Button
              ...
              onClick={handleState('all', true)}
              backgroundColor="warning"
            />
            <Toast
              isOpened={Object.values(state).includes('all') ? state['isOpened'] : false}
              onOpenChange={handleState('all', false)}
              jsxElement={<Icon name="alert" />}
              title="Bot Anik"
              description="Bot Anik is an appreciated member of the OKP4 team, helping to enforce a DevOps culture!"
              severityLevel="warning"
            />
          </div>
        </div>
