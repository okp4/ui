...
>
  {args => {
    ...
    return (
      ...
      >
        <Stepper {...args} onSubmit={() => setSubmitted(true)} />
        <Toast
          isOpened={submitted}
          onOpenChange={() => setSubmitted(false)}
...
The `Stepper` component comes with a list of `Step`. A `Step` must respect the following type :
          ...
          content: (
            <div>
              ...
              >
                ...
                >
                  <source
                    src="https://okp4.network/wp-content/themes/okp4/img/isometric-okp42.mp4"
                    type="video/mp4"
                  />
                  <source
                    src="https://okp4.network/wp-content/themes/okp4/img/isometric-okp42.mp4"
...
<Canvas>
  <Story name="Submit button label">
    {() => {
      const steps = [
        {
          label: 'Q1',
      ...
      return (
        ...
        >
          <Stepper steps={steps} active={3} submitButtonLabel="End of reading" />
        </div>
      )
    }}
  </Story>
...
## Callback methods `onNext` and `onSubmit`

The `Stepper` component provide the `onNext` and `onSubmit` properties.<br />
...
<Canvas>
  <Story name="onNext and onSubmit">
    {() => {
      ...
      return (
        ...
        >
          <Stepper
            steps={steps}
            onNext={() => setIsNextOpened(true)}
            onSubmit={() => setIsSubmitOpened(true)}
          />
          <Toast
            description="You clicked on Next button"
            isOpened={isNextOpened}
            onOpenChange={() => setIsNextOpened(false)}
            title="onNext"
            severityLevel="success"
          />
          <Toast
            ...
            onOpenChange={() => setIsSubmitOpened(false)}
            title="onSubmit"
            severityLevel="success"
          />
        </div>
