...
The `Stepper` comes with a list of steps. A `Step` must respect the following type :

```ts
type Step = {
  readonly label: string
  readonly status?: 'disabled' | 'invalid' | 'completed' | 'active' | 'uncompleted'
...
It is possible to provide an initial status to each step. The initial status is one of the following values : `disabled`, `invalid`, `completed`, `active` or `uncompleted`.  
By default, a step is `uncompleted`.

- `disabled` - The step is disabled and can't be reached when clicked on the _Previous_ or the _Next_ button.
- `invalid` - The step is in error. When it is reached, it becomes active. Then it becomes `uncompleted` if you go back to a previous step, `completed` if you go to the next step, or `invalid` if an error is detected (see the [onValidate](/docs/molecules-stepper--on-validate) section).
...
Users can provide a label for the submit button with the `submitButtonLabel` property and for the reset button with the `resetButtonLabel` property.
...
The `successContent` property is used to provide a `JSX.Element` content that is displayed at the end when the last step is completed.

<Canvas>
  <Story name="Success content">
    {() => {
      const steps = [
        {
          label: 'ØKP4 Protocol',
      ...
      return (
        ...
        >
          <Stepper
            steps={steps}
            submitButtonLabel="Finish reading"
            resetButtonLabel="Read again"
            successContent={
              <div>
                <Typography as="div" fontFamily="brand" fontWeight="light" color="success">
                  You're done reading everything !
                </Typography>
                <Typography as="div" fontFamily="brand" fontWeight="light">
...
The `Stepper` provides the `onNext` and `onSubmit` properties.  
The callback method `onNext` is called when clicking on the _Next_ button.  
The callback method `onSubmit` is called when clicking on the _Submit_ button in the last step.  
Once the last step is completed, the `Reset` button appears. The callback method `onReset` is called when clicking on this button.  
The `Reset` corresponds to the reset of the stepper to its initial state (as it was originally set up by the user).
