...
type Step = {
  ...
  readonly status?: 'disabled' | 'error' | 'completed' | 'active' | 'uncompleted'
...
It is possible to provide an initial status to each step. The initial status is one of the following values : `disabled`, `error`, `completed`, `active` or `uncompleted`.  
