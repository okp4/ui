...
import {
  ...
  getUpdatedSteps,
  getCurrentStep
} from 'ui/molecules/stepper/selector/stepper.selector'
import './stepper.scss'
...
Each step can be validated to move on to the next step. The last step also provides the possibility to perform an additional action upon validation. Once all steps are completed, the user can reset the stepper.  
...
## Stepper with `useStepper` hook and selectors
...
Along with this hook, the library provides selectors. These selectors are functions that retrieve useful data from the state returned by the `useStepper` hook.

We recommend using the `useStepper` hook with the selector functions to make it easier to use the Stepper. But you can also implement your own.
...
The `useStepper` hook gives you :

- The `state` object. It contains the following values:
...
- The `dispatch` funtion. It allows to dispatch one of the following actions :
  - `previousClicked` : sets the current step as **uncompleted** (unless if it is **invalid**) and then sets the previous step as current.
...
To call the `useStepper` hook, or to dispatch the `stepperReset` action, you must provide two parameters :

- the id of the current initial step
- the initial status list
...
The `@okp4/ui` library provides the following selector to facilitate the use of the hook with the Stepper :

- `getUpdatedSteps`: Allows to get the steps with their updated status thanks to the state returned by the `useStepper` hook.

### Example

Here is a complete example of how to use the Stepper with the `useStepper` hook and selectors.
...
Finally, return the Stepper with the appropriated props and the help of selector function
    ...
    {() => {
      ...
      const handleReset = useCallback(() => {
        dispatch({
          type: 'stepperReset',
          payload: {
            initialCurrentStepId: 'personal-information',
            initialStepsStatus: steps.map(step => ({ id: step.id, status: step.status }))
          }
        })
      }, [dispatch, steps])
...
- **invalid** - The step is in error. When it is reached, it becomes active. Then it becomes **completed** if you go to the next step without error, or **invalid** if an error is detected (see the [onValidate](/docs/molecules-stepper--on-validate) section).
    ...
    {() => {
      const steps = [
        ...
        },
        {
          ...
          content: (
            <Typography as="div" fontFamily="brand" fontSize="small" fontWeight="xlight">
              This step was in error. It becomes active when switching on it. It remains in error if
              you click on Previous or becomes complete if click on Next.
            </Typography>
          ),
          status: 'invalid'
        },
...
The `isSubmitButtonDisabled` optional property allows to disabled the _Submit_ button.
...
Users can provide a label for the _Submit_ button with the `submitButtonLabel` property and for the _Reset_ button with the `resetButtonLabel` property.
...
The callback function `onPrevious` is called when clicking on the _Previous_ button.  
The callback function `onNext` is called when clicking on the _Next_ button.  
The callback function `onSubmit` is called when clicking on the _Submit_ button in the last step.  
Once the last step is completed, the _Reset_ button appears. The callback function `onReset` is called when clicking on this button.
...
<Canvas>
  <Story name="Callback functions">
    {() => {
      ...
      const handlePreviousOpened = bool => () => setNextOpened(bool)
