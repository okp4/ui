...
A list of options of `Option` type must be provided in order to use `Select`.  
...
We provide you with two alphabetical sorting methods, to help you present the options, that you can import from the `utils.ts` file:
