...
The `Select` component can be readOnly by setting the `readOnly` property to `true`.  
It is then impossible to change the value or to open the menu, but the value should still be submitted to its parent.
