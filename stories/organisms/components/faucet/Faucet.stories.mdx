...
To work, the component uses [redux](https://redux.js.org/) as a state manager.  
To do this, all you have to do is declare the store instances in a `store.ts` file to be able to import those parameters later into a `Provider` component that will take care of injecting them into the React context.
...
Finally, it is necessary to wrap the `Faucet` component in a `StoreProvider` which will inject into the React context the configuration elements necessary for its proper functioning.
This `StoreProvider` component is directly provided by the `@okp4/ui` library, you simply have to pass it the parameters returned by the `store.ts` file as properties.
...
- Keep in mind that the OKP4 Faucet is only available within a `testnet` environment, it cannot be used in the `mainnet` environment to get some know 😉
